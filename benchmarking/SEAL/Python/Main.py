...
for epoch in range(cmd_args.num_epochs):
    ...
    if not cmd_args.printAUC:
        avg_loss[2] = 0.0
    print('\033[92maverage training of epoch %d: loss %.5f acc %.5f roc_auc %.5f pr_auc %.5f\033[0m' % (
        epoch + 1, avg_loss[0], avg_loss[1], avg_loss[2], avg_loss[3]))

    classifier.eval()
    val_loss = loop_dataset(val_graphs, classifier, list(range(len(val_graphs))))
    if not cmd_args.printAUC:
        val_loss[2] = 0.0
    print('\033[93maverage validation of epoch %d: loss %.5f acc %.5f roc_auc %.5f pr_auc %.5f\033[0m' % (
        epoch + 1, val_loss[0], val_loss[1], val_loss[2], val_loss[3]))
    ...
    if val_loss[0] <= best_loss[0]:
        ...
        if not cmd_args.printAUC:
            test_loss[2] = 0.0
        print('\033[94maverage test of epoch %d: loss %.5f acc %.5f roc_auc %.5f pr_auc %.5f\033[0m' % (
            epoch + 1, test_loss[0], test_loss[1], test_loss[2], test_loss[3]))

print('\033[95mFinal test performance: epoch %d: loss %.5f acc %.5f roc_auc %.5f pr_auc %.5f\033[0m' % (
    best_epoch + 1, test_loss[0], test_loss[1], test_loss[2], test_loss[3]))
